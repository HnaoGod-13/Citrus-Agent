...
export const intakeStatus = status => ({draft:'草稿',submitted:'待审核',approved:'审核通过',needs_changes:'退回补充',rejected:'审核不通过',archived:'已归档'}[status] || '未保存');
...
export function createIntakeController({root, model, data, setStateValue, render, flash, download, persist, changeView}) {
  ...
  function markup() {
    ...
    const status=(state.message?`<div class="feedback" role="status">${e(state.message)}</div>`:'')+(side && current().review_note?`<div class="feedback" role="status">${intakeStatus(current().status)} · 审核意见：${e(current().review_note)}</div>`:'');
