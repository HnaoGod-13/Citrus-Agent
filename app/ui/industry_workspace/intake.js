...
export function createIntakeController({root, model, data, setStateValue, render, flash, download, persist, changeView}) {
  ...
  function markup() {
    ...
    if (!side) return headline+nav+status+`<div class="ic-entry-grid">${Object.entries(schema.sides).map(([key,s])=>`<button type="button" class="panel ic-entry" data-action="ic-side-${key}"><span class="ic-entry-symbol">${key==='supplier'?'01':'02'}</span><h2>${e(s.title)}</h2><p>${e(s.subtitle)}</p><div class="ic-entry-tags">${(key==='supplier'?['种植基地','农业投入品','原料品质','采收供应']:['批次接收','实际加工参数','成品质检','产出追溯']).map(t=>`<span>${t}</span>`).join('')}</div><b>进入采集 →</b></button>`).join('')}</div><section class="panel ic-guidance"><h2>一份批次档案，贯通两端信息</h2><p>供应端记录“怎样种出来”，生产端记录“用什么原料、怎样加工、最终品质如何”。保存供应批次后，可在生产端的原料接收板块关联带入。</p><p>可随时保存草稿、补充明细与检测附件，完成后提交待复核记录。</p><label class="ic-upload">导入采集备份<input type="file" data-intake-upload="backup" aria-label="导入采集备份" accept=".json"></label></section>`;
