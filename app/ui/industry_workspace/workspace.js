...
const header=(code,title)=>`<header class="page-head"><div class="eyebrow">CITRUS AI · ${code}</div><h1>${title}</h1></header>`;
...
export default function(component) {
  ...
  const initialTab=view==='market'?(model.marketOpenTab??0):0;
  if(root._view!==view&&view==='market')delete model.marketOpenTab;
  ...
  const intakeController=createIntakeController({root,model,data,setStateValue,render,flash,download:downloadFile,persist,changeView:changeLocalView});

  function dataCollection() {
    return intakeController ? intakeController.render() : '<p>采集表加载中，请刷新后重试。</p>';
  }
  ...
  function supply(embedded=false) {
    const active=embedded?0:tab;
    ...
    if(active===1)body=`<section class="panel audit"><h2>匹配买家</h2><div class="equipment-row"><span>NFC 果汁加工原料采购</span><span>15—25 吨 · 糖度 ≥ 12.0 °Brix</span>${button('查看匹配','show-matches')}</div>${notice('演示匹配','以上为示例采购需求，不代表真实买家询盘。')}</section>`;
    if(active===2)body=`<section class="panel audit"><h2>发布记录</h2>${model.supplies.length?model.supplies.map(s=>`<div class="equipment-row">${esc(s.name)} · ${esc(s.batch)} ${pill('会话草稿')}</div>`).join(''):'<p class="empty">本次会话尚无新建供应草稿。</p>'}</section>`;
  ...
  function demand(embedded=false) {
    ...
    if(!embedded&&tab===0)body=`<section class="panel audit"><h2>我的需求</h2><div class="equipment-row"><span>${esc(r.name)}</span>${pill(r.published?'会话演示 · 已发布':'草稿')}${button('继续编辑','edit-demand')}${button('查看匹配','show-matches')}</div></section>`;
    if(!embedded&&tab===2)body=`<section class="panel audit"><h2>历史模板</h2><div class="equipment-row"><span>NFC 果汁加工原料采购 · 示例模板</span>${button('使用模板','use-template')}</div><p>使用模板会替换当前草稿，使用前可确认。</p></section>`;
    if(embedded)return body;
  ...
  function matching(embedded=false, section=tab) {
    const r=model.request;
    ...
    const trade=`<section class="panel trade-demand"><div><span class="eyebrow">采购方 · 重庆北碚示例加工厂</span><h2>NFC 果汁加工原料跨区域采购</h2><p>柑橘鲜果 · 80—120 吨 · 糖度 ≥ 11.8 °Brix · 11.15 前到厂</p></div><div class="trade-route">山东 / 江西 / 广西 <b>→</b> 重庆</div></section><div class="trade-grid">${tradeCandidates.map((c,i)=>`<article class="panel trade-card"><div class="trade-title"><h2>${esc(c.seller)}</h2>${pill(c.label,c.score?i===0?'black':'':'red')}</div><p>${esc(c.origin)} · ${esc(c.material)}</p><div class="trade-score">${c.score?`<b>${c.score}</b><span>商业适配度</span>`:'<b>—</b><span>暂不推荐</span>'}</div><dl><div><dt>可供数量</dt><dd>${c.quantity} 吨</dd></div><div><dt>糖度</dt><dd>${c.brix} °Brix</dd></div><div><dt>预计到厂</dt><dd>${c.arrival}</dd></div><div><dt>资料</dt><dd>${c.docs}</dd></div></dl>${button(c.score?'申请商业对接':'数量不满足',`trade-${c.id}`,i===0,'link',c.score?'':'disabled')}</article>`).join('')}</div>`;
    const records=`<section class="panel audit"><h2>对接记录</h2>${model.tradeConnections?.length?model.tradeConnections.map(id=>`<div class="equipment-row"><span>${esc(id)} · 重庆采购场景</span>${pill('申请草稿')}</div>`).join(''):'<p class="empty">暂无商业对接申请。</p>'}</section>`;
    const body=section===0?current:section===1?trade:records;
    if(embedded)return body;
    return header('CONNECTIONS','商业对接','连接跨区域原料供应与加工需求，先筛条件再谈交易')+`<div class="toolbar">${tabs(['当前需求匹配','跨区域商机','对接记录'],tab)}</div>`+body;
  }

  function market() {
    const actions=tab===0?button('导入采集批次','import-supply',false,'upload')+button('发布供应','new-supply',true,'plus'):
      tab===1?button('保存需求草稿','save-demand')+button('发布需求','publish-demand',true):
      tab===2?button('按当前需求重新匹配','rematch',true,'filter'):'';
    const bodies=[supply(true),demand(true),matching(true,0),matching(true,1),matching(true,2)];
    return header('SUPPLY & DEMAND','产销对接中心')+`<div class="toolbar market-toolbar">${tabs(['供应资源','采购需求','智能匹配','跨区商机','对接记录'],tab)}<div class="actions">${actions}</div></div>${bodies[tab]||bodies[0]}`;
  }
  ...
  function reportWorkSummary() {
    ...
    if(model.activeIntakeReport){
      ...
      const audit=intakeAudit(d,data.intakeSchema);
      return {batch:supplier?f['harvest.batch']||'待补充':r.batch||'待补充',
        material:supplier?`${f['base.variety']||'待补充'} · ${f['base.origin']||'产地待补充'}`:`${r.name||'待补充'} · ${r.origin||'产地待补充'}`,
        quantity:supplier?`${f['harvest.quantity']||'待补充'} ${f['harvest.unit']||''}`:`${r.input||'投料量待补充'} ${r.unit||''}`,
        ...
        issues:[...audit.errors.map(x=>x.message),'记录为用户填报，检测和工艺结论尚需单位复核。'],route:route.label||'待补充',routeScore:route.score??'—',dataQuality:cleaning.weighted_score??audit.score,researchStatus:enrichment.research_status||'待补充',varietyInsight:enrichment.variety_insight||'待补充',researchSources:(enrichment.evidence||[]).slice(0,6).map(x=>x.title||x.document_title||x.source||'本地知识条目')};
    }
    ...
    return {
      ...
      issues,route:route.label||'待补充',routeScore:route.score??'—',dataQuality:cleaning.weighted_score??audit.score,researchStatus:enrichment.research_status||'待补充',varietyInsight:enrichment.variety_insight||'待补充',researchSources:(enrichment.evidence||[]).slice(0,6).map(x=>x.title||x.document_title||x.source||'本地知识条目')
    };
  }
  ...
  function reportPreview() {
    ...
    return `<div class="report-paper"><div class="report-cover"><span>${esc(r.agency)} · ${esc(r.department||'')}</span><h2>${esc(r.title)}</h2><p>${esc(r.period)} · ${esc(r.region)} · ${esc(r.reportType||'项目报告')}</p>${r.generated?`<b class="report-generated-mark">已生成可编辑项目报告</b>`:''}</div><div class="report-body">${generated||`<h3>报告摘要</h3><p>正式提交后，将围绕本批次的产地、品种、质量、工艺路线和建设条件形成项目报告，并结合公开产业资料完善产业、市场和政策分析。</p><h3>当前批次概览</h3><div class="report-summary-list"><div><span>批次</span><b>${esc(s.batch)}</b></div><div><span>原料与数量</span><b>${esc(s.material)} · ${esc(s.quantity)}</b></div><div><span>加工任务</span><b>${esc(s.processing)}</b></div><div><span>资料状态</span><b>信息已汇总</b></div></div><small>请先正式提交采集记录，再生成项目报告。</small>`}</div></div>`;
  }
  function reports() {
    const r=model.report;
    const s=reportWorkSummary();
  ...
  function visuals() {
    return renderIndustryVisuals({tab,raw:data.intakeAnalytics,request:model.request,header,tabs,button,svg,esc});
  }
  function render() {
    root._view=view;root._tab=tab;root._selected=selected;root._listMode=listMode;
    root.innerHTML=`<div class="page" data-view="${view}">${feedback?`<div class="feedback" role="status">${esc(feedback)}</div>`:''}${({data:dataCollection,market,visuals,reports})[view]()}</div><dialog aria-label="业务操作"></dialog>`;
  }
  ...
  function onClick(event) {
    ...
    if(intakeController?.click(el))return;
    ...
    if(action==='edit-demand'){if(view==='market'){tab=1;render();}else{model.marketOpenTab=1;changeLocalView('market');}return;}
    if(action==='show-matches'||action?.startsWith('buyers-')){if(view==='market'){tab=2;render();}else{model.marketOpenTab=2;changeLocalView('market');}return;}
  ...
  function onInput(event) {
    ...
    if(intakeController?.input(el))return;
