...
export default function(component) {
  ...
  function reportPreview() {
    ...
    return `<div class="report-paper"><div class="report-cover"><span>${esc(r.agency)} · ${esc(r.department||'')}</span><h2>${esc(r.title)}</h2><p>${esc(r.period)} · ${esc(r.region)} · ${esc(r.reportType||'项目报告')}</p>${r.generated?`<b class="report-generated-mark">已生成可编辑项目报告</b>`:''}</div><div class="report-body">${generated||`<h3>报告摘要</h3><p>提交本批次信息后，将围绕产地、品种、质量、工艺路线和建设条件形成项目报告，并结合公开产业资料完善产业、市场和政策分析。</p><h3>当前批次概览</h3><div class="report-summary-list"><div><span>批次</span><b>${esc(s.batch)}</b></div><div><span>原料与数量</span><b>${esc(s.material)} · ${esc(s.quantity)}</b></div><div><span>加工任务</span><b>${esc(s.processing)}</b></div><div><span>资料状态</span><b>信息已汇总</b></div></div><small>请先提交本批次信息，再生成项目报告。</small>`}</div></div>`;
  }
  function reports() {
    const r=model.report;
    const s=reportWorkSummary();
